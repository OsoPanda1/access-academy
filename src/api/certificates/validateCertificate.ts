 ...
 * Endpoint público para validar certificados UTAMV
 * Permite verificar autenticidad mediante certificate_hash
 */
...
export async function validateCertificate(req: Request, res: Response) {
  try {
    ...
    if (error || !data) {
      return res.status(404).json({ valid: false, message: "Certificado no encontrado o inválido." });
    }

    // Validación académica
    if (!data.is_valid) {
      return res.status(200).json({ valid: false, message: "Certificado revocado o expirado." });
    }
